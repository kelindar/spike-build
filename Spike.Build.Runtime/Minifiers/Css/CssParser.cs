...
namespace Spike.Build.Minifiers
{
    ...
    internal class CssParser
    {
        ...
        private JsSettings m_jsSettings;
        public JsSettings JSSettings
        {
            get
            {
                return m_jsSettings;
            ...
            set
            {
                if (value != null)
                {
                    ...
                    m_jsSettings.SourceMode = JsSourceMode.Expression;
                }
                else
                {
                    m_jsSettings = new JsSettings()
                        {
                            KillSwitch = (long)JsTreeModifications.MinifyStringLiterals,
                            SourceMode = JsSourceMode.Expression
                        };
                }
            }
        }
        ...
        */
        ...
        private Parsed ParseKeyFrames()
        {
            ...
            if (CurrentTokenType == TokenType.KeyFramesSymbol)
            {
                ...
                if (CurrentTokenType == TokenType.Identifier || CurrentTokenType == TokenType.String)
                {
                    ...
                    if (CurrentTokenType == TokenType.Identifier || Settings.OutputMode == MinifierOutputMode.MultipleLines)
                    {
                        Append(' ');
                    }
                ...
                if (CurrentTokenType == TokenType.Character && CurrentTokenText == "{")
                {
                    if (Settings.BlocksStartOnSameLine == MinifierBlockStart.NewLine
                        || Settings.BlocksStartOnSameLine == MinifierBlockStart.UseSource && m_encounteredNewLine)
                    {
                        NewLine();
                    }
                    else if (Settings.OutputMode == MinifierOutputMode.MultipleLines)
                    {
                        Append(' ');
                    }
        ...
        private Parsed ParseKeyFrameSelectors()
        {
            ...
            while (parsed == Parsed.True && CurrentTokenType == TokenType.Character && CurrentTokenText == ",")
            {
                ...
                AppendCurrent();
                if (Settings.OutputMode == MinifierOutputMode.MultipleLines)
                {
                    Append(' ');
                }
                SkipSpace();
        ...
        private Parsed ParseImport()
        {
            ...
            if (CurrentTokenType == TokenType.ImportSymbol)
            {
                ...
                else
                {
                    ...
                    if (CurrentTokenType == TokenType.Uri || Settings.OutputMode == MinifierOutputMode.MultipleLines)
                    {
                        Append(' ');
                    }
        ...
        private Parsed ParseMediaQuery(bool firstQuery)
        {
                ...
                || string.Compare(CurrentTokenText, "NOT", StringComparison.OrdinalIgnoreCase) == 0))
            {
                ...
                if (firstQuery || Settings.OutputMode == MinifierOutputMode.MultipleLines)
                {
                    Append(' ');
                }
            ...
            if (CurrentTokenType == TokenType.Identifier)
            {
                ...
                if (mightNeedSpace || Settings.OutputMode == MinifierOutputMode.MultipleLines)
                {
                    Append(' ');
                }
            ...
            if (!m_noOutput)
            {
                ...
                else
                {
                    ...
                    if (m_forceNewLine)
                    {
                        ...
                        if (!m_outputNewLine && Settings.OutputMode == MinifierOutputMode.MultipleLines)
                        {
                            AddNewLine();
                        }
        ...
        private void NewLine()
        {
            ...
            if (Settings.OutputMode == MinifierOutputMode.MultipleLines && !m_outputNewLine)
            {
                AddNewLine();
                m_lineLength = 0;
                m_outputNewLine = true;
...
//            RegexOptions.IgnoreCase | RegexOptions.Singleline | RegexOptions.Compiled);
        ...
        string NormalizeImportantComment(string source)
        {
            ...
            if (Settings.OutputMode == MinifierOutputMode.SingleLine)
            {
    ...
    internal sealed class CssException
    {
    ...
    internal class CssErrorEventArgs : EventArgs
    {
        /// <summary>
