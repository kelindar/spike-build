...
{
    internal class JsAnalyzeNodeVisitor : JsTreeVisitor
    {
        private JsParser m_parser;
        private bool m_encounteredCCOn;// = false;
        private JsMatchPropertiesVisitor m_matchVisitor;// == null;
        private Stack<JsActivationObject> m_scopeStack;

        public JsAnalyzeNodeVisitor(JsParser parser)
        {
            m_parser = parser;
            m_scopeStack = new Stack<JsActivationObject>();
        ...
        private static void CombineWithPreviousVar(JsBlock node, int ndx, JsVar previousVar)
        {
            var binaryOp = node[ndx] as JsBinaryOperator;
            JsLookup lookup;
            if (binaryOp != null
                && binaryOp.IsAssign
                && (lookup = binaryOp.Operand1 as JsLookup) != null
        ...
        private static JsAstNode FindLastStatement(JsBlock node)
        {
            ...
            while (lastStatementIndex >= 0 
                && (node[lastStatementIndex] is JsFunctionObject || node[lastStatementIndex] is JsImportantComment))
            {
                --lastStatementIndex;
            }
        ...
        private static bool LabelMatchesParent(string label, JsAstNode parentNode)
        {
            var isMatch = false;

            // see if the parent's parent is a labeled statement
            JsLabeledStatement labeledStatement;
            while ((labeledStatement = parentNode.Parent as JsLabeledStatement) != null)
            {
        ...
        private static JsIfNode IsIfReturnExpr(JsAstNode node, out JsAstNode condition, ref JsAstNode matchExpression)
        {
            // set the condition to null initially
            condition = null;

            // must be an if-node with no false block, and a true block with one statement
            var ifNode = node as JsIfNode;
            if (ifNode != null
                && ifNode.FalseBlock == null
                && ifNode.TrueBlock != null
                && ifNode.TrueBlock.Count == 1)
        ...
        private static int PreviousStatementIndex(JsBlock node, JsAstNode child)
        {
            // get the index of the statement before the last return
            // (skip over function decls and importand comments)
            var indexPrevious = node.IndexOf(child) - 1;
            while (indexPrevious >= 0 && (node[indexPrevious] is JsFunctionObject || node[indexPrevious] is JsImportantComment))
            {
                --indexPrevious;
            }
        ...
        public override void Visit(JsConditionalCompilationOn node)
        {
        ...
        public override void Visit(JsForIn node)
        {
            if (node != null)
            {
        ...
        public override void Visit(JsLookup node)
        {
            if (node != null)
            {
                ...
                if (node.Parent is JsCallNode)
                {
                    node.RefType = (
                      ((JsCallNode)(node.Parent)).IsConstructor
                      ? JsReferenceType.Constructor
                      : JsReferenceType.Function
                      );
                }
                ...
                if (node.VariableField != null && node.VariableField.FieldType == JsFieldType.Predefined)
                {
                    ...
                    if (string.CompareOrdinal(node.Name, "NaN") == 0)
                    {
                        ...
                        node.Parent.ReplaceChild(node, new JsConstantWrapper(double.NaN, JsPrimitiveType.Number, node.Context, m_parser));
                    }
                    else if (string.CompareOrdinal(node.Name, "Infinity") == 0)
                    {
                        // don't analyze the new ConstantWrapper -- we don't want it to take part in the
                        // duplicate constant combination logic should it be turned on.
                        node.Parent.ReplaceChild(node, new JsConstantWrapper(double.PositiveInfinity, JsPrimitiveType.Number, node.Context, m_parser));
                    }
                }
            }
        }
        ...
        public override void Visit(JsObjectLiteralField node)
        {
            if (node != null)
            {
                if (node.PrimitiveType == JsPrimitiveType.String
        ...
        private static string GuessAtName(JsAstNode node)
        {
            ...
            if (parent != null)
            {
                ...
                JsCallNode call = parent as JsCallNode;
        ...
        private static bool VarDeclExists(JsVar node, int ndx, string name)
        {
        ...
        private JsUnaryOperator CreateVoidNode()
        {
            return new JsUnaryOperator(null, m_parser)
                {
                    Operand = new JsConstantWrapper(0.0, JsPrimitiveType.Number, null, m_parser),
