...
{
    internal class JsReorderScopeVisitor : JsTreeVisitor
    {
        // list of all function declarations found in this scope
        private List<JsFunctionObject> m_functionDeclarations;

        // list of all other functions found in this scope
        private List<JsFunctionObject> m_functionExpressions;
        ...
        private List<JsDirectivePrologue> m_moduleDirectives;

        // list of all var statements found in this scope
        private List<JsVar> m_varStatements;
        ...
        public override void Visit(JsConditionalCompilationEnd node)
        {
            if (node != null)
            {
        ...
        public override void Visit(JsDirectivePrologue node)
        {
            if (node != null)
            {
                ...
                else
                {
                    ...
                    {
                        m_moduleDirectives = new List<JsDirectivePrologue>();
                    }
        ...
        public override void Visit(JsVar node)
        {
            if (node != null)
            {
                ...
                if (m_moveVarStatements && m_conditionalCommentLevel == 0)
                {
                    ...
                    {
                        m_varStatements = new List<JsVar>();
                    }
        ...
        public override void Visit(JsGroupingOperator node)
        {
            if (node != null)
            {
                ...
                if (node.Parent != null)
                {
                    ...
                    else if (node.Parent is JsBlock)
                    {
                        ...
                        // all others get axed.
                        if (!(node.Operand is JsFunctionObject) && !(node.Operand is JsObjectLiteral))
                        {
                            // delete self
                            deleteParens = true;
                        }
                    }
                    else if (node.Parent is JsAstNodeList)
                    {
                        ...
                        var binOp = node.Operand as JsBinaryOperator;
                    ...
                    else if (node.Parent.IsExpression)
                    {
                        ...
                        var conditional = node.Parent as JsConditional;
                        if (conditional != null)
                        {
